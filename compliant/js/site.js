...
(function(){
  ...
  const Cart = {
    ...
    totals(shipMethod="standard"){
      ...
      return {sub,discount,vol,ship,tax,total:sub-discount+ship+tax,code,promo,discountLabel:code?`Discount (${code})${vol?" + volume":""}`:"Volume discount"};
    },
...
<div class="totals"><div><span>Subtotal</span><b>${money(t.sub)}</b></div>${t.discount?`<div class="discount"><span>${t.discountLabel}</span><b>−${money(t.discount)}</b></div>`:""}</div>
