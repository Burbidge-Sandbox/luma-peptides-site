...
<div class="totals"><div><span>Subtotal</span><b>${money(t.sub)}</b></div>${t.discount?`<div class="discount"><span>${t.discountLabel}</span><b>−${money(t.discount)}</b></div>`:""}<div><span>Shipping</span><b>${t.ship?money(t.ship):"Free"}</b></div>${t.tax?`<div><span>Tax</span><b>${money(t.tax)}</b></div>`:""}<div class="grand"><span>Total</span><b>${money(t.total)}</b></div></div>`;
