(function(){
  let o=null; try{ o=JSON.parse(sessionStorage.getItem("luma_order")||"null"); }catch(e){}
 ...
 <div class="pay-step"><span class="step-n">1</span><div><b>Pay ${money(o.totals.total)} on Venmo</b><p>Send to <b>@${CFG.venmo.handle}</b> and put <b>${note}</b> in the note so the payment can be matched to this order.</p><a class="btn btn-primary" href="${payUrl}" target="_blank" rel="noopener">Open Venmo · ${money(o.totals.total)}</a> <a class="btn btn-ghost" href="${CFG.venmo.profile}" target="_blank" rel="noopener">View profile</a></div></div>
 ...
 <p class="pay-fine">Orders are released for shipping once the Venmo payment is received, usually within a few hours during business hours. Unpaid orders are cancelled after 48 hours. Screenshot or save this page: order <b>${o.id}</b>.</p>`;
  el.innerHTML=`${o.items.map(i=>`<div class="coa-row"><span>${i.qty}× ${i.name} <small style="color:var(--muted)">${i.strength} · ${i.plan==="subscribe"?"Monthly":"One-time"}</small></span><b>${money(i.price*i.qty)}</b></div>`).join("")}
<div class="coa-row"><span>Subtotal</span><b>${money(o.totals.sub)}</b></div>${o.totals.discount?`<div class="coa-row"><span>Discount (${o.totals.code})</span><b class="pass">−${money(o.totals.discount)}</b></div>`:""}<div class="coa-row"><span>Shipping · ${o.ship}</span><b>${o.totals.ship?money(o.totals.ship):"Free"}</b></div><div class="coa-row"><span><b>Total</b> · Venmo, payment pending</span><b>${money(o.totals.total)}</b></div>
<p style="font-size:.85rem;color:var(--muted);margin:1rem 0 0"><b>Ships to:</b> ${o.name}, ${o.address}</p>
